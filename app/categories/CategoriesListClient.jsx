...
import { PILLAR_META } from "./categoriesData";
...
function categorySortValue(row, key) {
  if (key === "itemsInside") return row.itemsInside;
  if (key === "pillar") return PILLAR_META[row.pillarKey].label.toLowerCase();
  return String(row[key] ?? "").toLowerCase();
}

export function CategoriesListClient({ initialRows }) {
  ...
  const { sortKey, sortDir, toggleSort } = useTableSort("category", "asc");
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return pillar.includes(q) || r.category.toLowerCase().includes(q) || String(r.itemsInside).includes(q);
    });
  }, [rows, query]);

  const sorted = useMemo(() => sortRows(filtered, sortKey, sortDir, categorySortValue), [filtered, sortKey, sortDir]);
  ...
  return (
    <>
      {toastOpen ? <SuccessToast message="New Category Added Successfully!" onDismiss={() => setToastOpen(false)} /> : null}
      ...
      <div className="flex-1 overflow-auto p-6 lg:p-8">
        <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Categories</h1>
              <p className="mt-2 max-w-2xl text-sm text-gray-600 sm:text-base">
                This list of categories allow you to keep items organized and serve as a filter when searching for items to add to a protocol.
              </p>
            </div>
            <Link href="/categories/new" className="btn-lifemuse-primary shrink-0 self-start sm:self-center">
              Create New Category
            </Link>
          </div>
              ...
              <table className="w-full min-w-[720px] table-fixed text-left text-sm">
                ...
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/90 text-xs font-semibold uppercase tracking-wide text-gray-500">
                    <SortableTh columnKey="pillar" label="Pillar" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} className="w-[14%] whitespace-nowrap px-4 py-3 sm:px-5" />
                    <SortableTh columnKey="category" label="Category" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} className="w-[68%] px-4 py-3 sm:px-5" />
                    <SortableTh columnKey="itemsInside" label="Items Inside" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} className="w-[12%] whitespace-nowrap px-4 py-3 sm:px-5" />
                ...
                <tbody>
                  {pageSlice.map((row, i) => (
                    <tr key={row.id} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/60"}`}>
                      ...
                      <td className="min-w-0 px-4 py-3.5 align-middle font-medium text-gray-900 sm:px-5">
                        <div className="truncate" title={row.category}>
                          {row.category}
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-4 py-3.5 align-middle tabular-nums text-gray-700 sm:px-5">{row.itemsInside}</td>
