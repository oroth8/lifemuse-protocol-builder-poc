...
function ProgressNoteStatusBadge({ status }) {
...
export function ProgressNoteDetailView({ member, note }) {
  const manageHref = `/members/${member.id}/progress-notes`;
        ...
        <div className="mx-auto max-w-6xl space-y-6">
          <PageBreadcrumb
            crumbs={[
              ...
              { label: "Manage Progress Notes", href: manageHref },
              { label: `Progress Note #${note.id}` },
            ]}
          />

          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <BackTitle href={manageHref}>Progress Note #{note.id}</BackTitle>
            <button
              type="button"
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm">
            <dl className="divide-y divide-gray-100">
              ...
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-center sm:px-8">
                ...
                <dd className="text-sm text-gray-800">{note.detailDate}</dd>
              </div>
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-center sm:px-8">
                <dt className="text-sm font-semibold text-gray-900">Status</dt>
                <dd>
                  <ProgressNoteStatusBadge status={note.status ?? "published"} />
                </dd>
              </div>
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-start sm:px-8">
                ...
                <dd className="whitespace-pre-wrap text-sm leading-relaxed text-gray-800">{note.note}</dd>
              </div>
            </dl>
          </div>
        </div>
