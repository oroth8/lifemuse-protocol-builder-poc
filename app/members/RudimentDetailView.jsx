...
function rudimentStatusLabel(status) {
  return status === "draft" ? "Draft" : "Published";
}

function RudimentStatusBadge({ status }) {
  ...
  return (
    ...
    >
      {rudimentStatusLabel(status)}
    </span>
  );
}

export function RudimentDetailView({ member, rudiment }) {
  const manageHref = `/members/${member.id}/rudiments`;
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm">
            <dl className="divide-y divide-gray-100">
              ...
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-center sm:px-8">
                ...
                <dd className="text-sm text-gray-800">{rudiment.detailDate}</dd>
              </div>
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-center sm:px-8">
                <dt className="text-sm font-semibold text-gray-900">Status</dt>
                <dd>
                  <RudimentStatusBadge status={rudiment.status ?? "published"} />
                </dd>
              </div>
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-start sm:px-8">
                <dt className="text-sm font-semibold text-gray-900">Taken By</dt>
                <dd className="text-sm text-gray-800">{rudiment.takenBy}</dd>
              </div>
              <div className="grid gap-1 px-6 py-4 sm:grid-cols-[160px_1fr] sm:items-start sm:px-8">
                <dt className="text-sm font-semibold text-gray-900">Note</dt>
                <dd className="whitespace-pre-wrap text-sm leading-relaxed text-gray-800">{rudiment.note}</dd>
              </div>
            </dl>
          </div>
        </div>
